...
import com.example.dblock.repository.LettuceRepository;
...
public class LettuceLockFacade {
    ...
    public void decrease(Long key, Long count) throws InterruptedException{
        // Inventory Service의 decrease연산 앞 뒤에 락을 걸고 해제하는 코드를 미리 추가함
        while (!lettuceRepository.lock(key)){
            Thread.sleep(100);
        } // 락 점유 시도, 성공 시 false, 실패시 true가 걸리므로 재점유를 시도하도록 반복문 설계
